...
@interface CTDeactivatePromotionView ()

@end

@implementation CTDeactivatePromotionView
...
//https://simfel.com/apptsvc/rest/promotions/retrievePromotionSATiersBySASL?serviceAccommodatorId=FTRFFD1&serviceLocationId=FTRFFD1&status=ACTIVE&startIndex=0&count=10
    
    NSDictionary *params=@{@"serviceAccommodatorId":[CTCommonMethods sharedInstance].selectSa, @"serviceLocationId":[CTCommonMethods sharedInstance].selectSl,@"status":@"ACTIVE", @"startIndex":@(0), @"count":@(10)};
    ...
    [operation setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if (responseObject && [responseObject isKindOfClass:[NSData class]])
        {
            ...
            for (NSInteger i = 0; i < [responseObject multipartArray].count; i = i+2) {
                ...
                if ([promoation.promotionStatus isEqualToString:@"ACTIVE"]) {
...
- (void)viewDidLoad {
    ...
    //UINib *cellNib = [UINib nibWithNibName:@"CTDeActivatePromotionCell" bundle:nil];
    //[tblActivate registerNib:cellNib forCellReuseIdentifier:@"Cell"];
...
-(void)tableView:(UITableView *)tableView expandCell:(UITableViewCell *)cell withIndexPath:(NSIndexPath *)indexPath
{
    ...
    [UIView animateWithDuration:.5 animations:
     ^{
        ...
        if ([promotion.promotionStatus isEqualToString:@"ACTIVE"]){
...
//             MainImage.image = [UIImage imageWithData:promotion.thumbnail];
//             TitleLbl.text = promotion.promotionSASLName;
//             MessgeLbl.text = [promotion.promotionType valueForKey:@"displayText"];
//             //MsgTitleLbl.text = promotion.promotionSASLName;
//             //OffersLbl.text = promotion.promotionSASLName;
//         }
         MainImage.alpha = 0;
...
//- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [cell.btnActivate addTarget:self action:@selector(Deactivate:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)Deactivate :(UIButton *)sender {
    int temp = sender.tag;
...
-(void)activatePromotion :(NSString *)promoUUID :(int)index
{
    ...
    NSString *promotionRequest = [NSString stringWithFormat:CT_PromotionDeactivate_URL,promoUUID,[CTCommonMethods sharedInstance].selectSa,[CTCommonMethods sharedInstance].selectSl,[CTCommonMethods UID]];
    ...
    AFJSONRequestOperation *operation=[AFJSONRequestOperation JSONRequestOperationWithRequest:request success:^(NSURLRequest *request, NSHTTPURLResponse *response, id JSON) {
        ...
        if (dict)
        {
            UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:StringAlertTitlePromotionDeactivated message:@"" delegate:self cancelButtonTitle:StringAlertButtonOK otherButtonTitles: nil];
            [alertView show];
