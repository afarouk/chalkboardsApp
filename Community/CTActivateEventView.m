...
@interface CTActivateEventView ()

@end

@implementation CTActivateEventView
    ...
    AFJSONRequestOperation *operation=[AFJSONRequestOperation JSONRequestOperationWithRequest:request success:^(NSURLRequest *request, NSHTTPURLResponse *response, id JSON) {
        ...
        if (JSON)
        {
            ...
            for (int k =0; k < listofevent.count; k++)
            {
                ...
                if ([[fetchvalue valueForKey:@"status"] isEqualToString:@"PROPOSED"]) {
...
-(void)activateAPI : (NSString *)eventId :(int)index
{
    ...
    NSString *promotionRequest = [NSString stringWithFormat:@"%@?UID=%@&serviceAccommodatorId=%@&serviceLocationId=%@&eventUUID=%@",CT_ActivateEventAPI_URL,[CTCommonMethods UID],[CTCommonMethods sharedInstance].selectSa,[CTCommonMethods sharedInstance].selectSl,eventId];
