...
- (void)setupContainerView
{
    ...
    if ([CTCommonMethods sharedInstance].NavFlag == NO)
    {
        NSLog(@"i'm flag");
        contentView.frame = CGRectMake(80, 0.0, CGRectGetWidth(self.containerView.frame), CGRectGetHeight(contentView.bounds));
        ...
        self.containerView.frame = CGRectMake(0.0, containerViewY, CGRectGetWidth(dropdownView.bounds)-85, self.contentViewHeight);
    }
    else
    {
        contentView.frame = CGRectMake(5, 0.0, CGRectGetWidth(self.containerView.frame), CGRectGetHeight(contentView.bounds));
        [self.containerView addSubview:contentView];
        CGFloat containerViewY = -self.contentViewHeight;
        if (self.dropdownViewController.dropdownPosition == MISDropdownViewControllerPositionBottom) {
            containerViewY = CGRectGetMaxY(dropdownView.frame);
...
- (CGFloat)visibleContainerViewYPosition
{
    ...
    if (self.dropdownViewController.dropdownPosition == MISDropdownViewControllerPositionTop) {
